...
if(isset($_POST['btn_sure'])){
    $email_arr=array();
    $res0=mysqli_query($link,"SELECT * FROM dbtest.users WHERE code=2");
    while ($row0=mysqli_fetch_array($res0)){
        $email_arr[]=$row0['email'];
    }
    if (in_array($email,$email_arr)) {
        $res1=mysqli_query($link,"UPDATE dbtest.users SET code=3 WHERE email='$email' AND code=2");
        $row=mysqli_fetch_array($res1);
        if ($res1){
//            $_SESSION['user'] = $row['user_id'];
            echo "<script language=JavaScript>window.alert('取消管理员成功！')</script>";
            echo "<script language=JavaScript>window.location='superhome.php'</script>";
        }else{
            echo "<script language=JavaScript>window.alert('取消管理员失败！')</script>";
        }
    }else{
